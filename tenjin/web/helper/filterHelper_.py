...
def get_filtered(project_id, name_filter, combo_filter, numeric_filter,
    ...
    name_query = {}
    if name_filter:
        name_query = {"NameLower": {"$regex": name_filter.lower()}}
    ...
    if "Experiments" in displayed_categories or "all" in displayed_categories:
        exps = get_db().db["Experiment"].find(
            {"$and": [
                project_query,
                {"$or": [
                    {"$and": [group_query_limiting, name_query, field_data_query, experiment_query,
                              date_created_query]},
    ...
    if "Samples" in displayed_categories or "all" in displayed_categories:
        samples = get_db().db["Sample"].find(
            {"$and": [
                project_query,
                {"$or": [
                    {"$and": [group_query_limiting, name_query, field_data_query, sample_query, date_created_query]},
                    group_query_additional
                ]}]}, {"_id": 1, "GroupIDList": 1}
        )
    ...
    if cat_id_list:
        researchitems = get_db().db["ResearchItem"].find(
            {"$and": [
                ...
                cat_query,
                {"$or": [
                    {"$and": [group_query_limiting, name_query, field_data_query, ri_query, date_created_query]},
                    group_query_additional
                ]}
            ]}, {"_id": 1, "GroupIDList": 1}
        )
