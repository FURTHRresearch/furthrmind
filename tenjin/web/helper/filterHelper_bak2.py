...
def get_project_index(projectId, group_id_list, nameFilter='', comboFilter=[], numericFilter=[],
                      groupFilter=[],
                      sampleFilter=[], experimentFilter=[], researchItemFilter=[],
                      checkFilter=[], add_field_data_id_list=True, recent=False,
                      index=1, include_linked="", date_filter=[], date_created=[],
                      text_filter=[], displayed_categories=[],
                      include_field_data=False, maxPage=None):
    filter_applied = check_if_filter_applied(nameFilter, comboFilter, numericFilter, groupFilter,
                                             sampleFilter, experimentFilter, researchItemFilter, checkFilter,
                                             date_filter, date_created, text_filter)

    if not filter_applied:
        s = _time.time()
        groups = get_index_no_filter(project_id=projectId, group_id_list=group_id_list, recent=recent,
        ...
        s = _time.time()
        if group_id_list:
            groups = get_filtered_index_with_group_id_list(projectId, group_id_list, nameFilter, comboFilter,
                                                           numericFilter,
                                                           groupFilter, sampleFilter, experimentFilter,
                                                           researchItemFilter,
                                                           checkFilter, recent, date_filter, date_created, text_filter,
                                                           include_field_data=include_field_data,
...
def get_index_no_filter(project_id, group_id_list, recent, include_field_data, displayed_categories):
    ...
    if not group_id_list:
        toplevel_groups = get_db().db["Group"].find({"ProjectID": project_id, "GroupID": None},
                                                    {"_id": 1, "Name": 1, "GroupID": 1, "ShortID": 1}).sort(
            order_by,
            direction).skip(
            start).limit(end)
    else:
        toplevel_groups = get_db().db["Group"].find({"ProjectID": project_id, "_id": {"$in": group_id_list}},
                                                    {"_id": 1, "Name": 1, "GroupID": 1, "ShortID": 1}).sort(
            order_by,
            direction).skip(
            start).limit(end)
    ...
    sub_groups = get_db().db["Group"].find({"_id": {"$in": sub_group_id_list}},
                                           {"_id": 1, "Name": 1, "GroupID": 1, "ShortID": 1}).sort(
        order_by,
        direction).skip(
        start).limit(end)
...
def get_groups_query_additional(field_data_query, name_query, date_created_query):
    ...
    groups = get_db().db["Group"].find({"$and": [
        group_query,
        date_created_query
    ]}, {"_id": 1})
...
def get_field_data_query_objects(combo_filter=None, numeric_filter=None, checks_filter=None, date_filter=None,
                                 text_filter=None):
    ...
    if combo_filter:
        ...
        for filter in combo_filter:
            field = filter["field"]
            field_data = get_db().db["FieldData"].find({"$and": [
                {"FieldID": bson.ObjectId(field)},
                {"Value": {"$in": [bson.ObjectId(i["id"]) for i in filter["values"]]}}
            ]}, {"_id": 1}
            )
    ...
    if numeric_filter:
        ...
        for nf in numeric_filter:
            ...
            field_dict = get_db().db["Field"].find_one({"_id": bson.ObjectId(field)}, {"_id": 1, "Type": 1})
            ...
            field_data = get_db().db["FieldData"].find(query, {"_id": 1}).hint(index_to_be_used)
    ...
    if date_filter:
        ...
        for df in date_filter:
            ...
            query = {"FieldID": field_id, "Value": {"$gte": value_min, "$lte": value_max}}
...
def get_linked_items_recursive(include_linked, exp_id_list, sample_id_list, researchitem_id_list):
    extra_exp_id_list = []
    ...
    if include_linked == "all":
        ...
        while True:
            temp_exp_id_list, temp_sample_id_list, temp_researchitem_id_list = get_linked_items(
            ...
            found_researchitem_id_list = list(set(temp_researchitem_id_list) - extra_researchitem_id_set)
            extra_researchitem_id_set.update(found_researchitem_id_list)

            if (not found_exp_id_list and not found_sample_id_list and not found_researchitem_id_list):
                break

        return list(extra_exp_id_set), list(extra_sample_id_set), list(extra_researchitem_id_set)
...
def create_result_list(groups, subgroup_mapping, group_item_mapping):
    ...
    for group in groups:
        ...
        add_subgroup_nodes(group["_id"], subgroup_mapping, group_item_mapping, result_list)
...
def add_subgroup_nodes(group_id, subgroup_mapping, group_item_mapping, result_list):
    if group_id in subgroup_mapping:
        for subgroup in subgroup_mapping[group_id]:
            ...
            result_list.append(sub_group_node)
            add_cat_nodes(result_list, subgroup, group_item_mapping)
            add_subgroup_nodes(subgroup["_id"], subgroup_mapping, group_item_mapping, result_list)
...
def get_all_parent_groups(group_id_list: list):
    groups = list(get_db().db["Group"].find({"_id": {"$in": group_id_list}}, {"_id": 1, "GroupID": 1}))
    ...
    while missing_group:
        ...
        if missing_group_id_list:
            new_groups = list(
                get_db().db["Group"].find({"_id": {"$in": missing_group_id_list}}, {"_id": 1, "GroupID": 1}))
...
def get_filtered_index_with_group_id_list(project_id, top_level_group_id_list, name_filter, combo_filter,
                                          numeric_filter, groups_filter,
                                          samples_filter, experiments_filter, researchitems_filter,
                                          checks_filter, recent, date_filter, date_created, text_filter,
                                          include_field_data, include_linked, displayed_categories, project_wide=False):
    top_level_group_id_list = [bson.ObjectId(g) for g in top_level_group_id_list]

    cat_id_list, category_mapping = get_valid_categories(project_id, displayed_categories)

    name_query_groups, show_as_empty_groups, exp_id_list, sample_id_list, researchitem_id_list = get_filtered(
        project_id, top_level_group_id_list, name_filter, combo_filter, numeric_filter,
        groups_filter, samples_filter, experiments_filter, researchitems_filter,
        checks_filter, recent, date_filter, date_created, text_filter,
        displayed_categories, include_linked, project_wide, cat_id_list)

    result_group_id_list, group_item_mapping = prepare_result_group_id_list_and_group_item_mapping(
        top_level_group_id_list, name_query_groups, show_as_empty_groups, exp_id_list, sample_id_list,
        researchitem_id_list, category_mapping
    )
    direction = pymongo.ASCENDING
    if recent:
        order_by = "Date"
    ...
    groups = get_db().db["Group"].find({"_id": {"$in": result_group_id_list}},
                                       {"_id": 1, "Name": 1, "GroupID": 1, "ShortID": 1}).sort(
        order_by,
        direction)
    top_level_groups = []
    ...
    result_list = create_result_list(top_level_groups, subgroup_mapping, group_item_mapping)
    return result_list


def get_page_group_mapping(project_id, nameFilter='', comboFilter=[], numericFilter=[], groupFilter=[],
                           sampleFilter=[], experimentFilter=[], researchItemFilter=[],
                           checkFilter=[], recent=False,
                           date_filter=[], date_created=[],
                           text_filter=[], displayed_categories=[], include_linked=False):
    ...
    if not filter_applied:
        s = _time.time()
        page_group_mapping = get_not_filtered_page_group_mapping(project_id=project_id, recent=recent,
                                                                 displayed_categories=displayed_categories)
        ...
        toplevel_groups = get_db().db["Group"].find({"ProjectID": project_id, "GroupID": None}, {"_id": 1}).sort(
            ...
            direction)
        group_id_list = [g["_id"] for g in toplevel_groups]
        page_group_mapping = get_filtered_page_group_mapping(project_id, group_id_list, nameFilter, comboFilter,
                                                             numericFilter,
                                                             groupFilter,
                                                             sampleFilter, experimentFilter, researchItemFilter,
                                                             checkFilter, recent, date_filter, date_created,
...
def get_not_filtered_page_group_mapping(project_id, recent, displayed_categories):
    ...
    toplevel_groups = get_db().db["Group"].find({"ProjectID": project_id, "GroupID": None}, {"_id": 1}).sort(order_by,
                                                                                                             direction)
...
def get_filtered_page_group_mapping(project_id, top_level_group_id_list, name_filter, combo_filter, numeric_filter,
                                    groups_filter, samples_filter, experiments_filter, researchitems_filter,
                                    checks_filter, recent, date_filter, date_created, text_filter,
                                    displayed_categories, include_linked):
    s = _time.time()
    ...
    toplevel_groups = get_db().db["Group"].find({"_id": {"$in": result_group_id_list}, "GroupID": None},
                                                {"_id": 1}).sort(
        order_by, direction)
...
def get_filtered(project_id, top_level_group_id_list, name_filter, combo_filter, numeric_filter,
                 groups_filter, samples_filter, experiments_filter, researchitems_filter,
                 checks_filter, recent, date_filter, date_created, text_filter,
                 displayed_categories, include_linked, project_wide, cat_id_list):
    s = _time.time()
    (group_id_list_including_subgroups, name_query_groups, show_as_empty_groups,
     unfiltered_group_id_list_including_subgroups) = filter_groups(project_id,
                                                                   top_level_group_id_list, groups_filter, name_filter,
                                                                   combo_filter, numeric_filter, checks_filter,
                                                                   date_filter, date_created, text_filter, project_wide)

    exp_id_list = []
    if not project_wide and ("Experiments" in displayed_categories or "all" in displayed_categories):
        exps = get_db().db["Experiment"].find({"GroupIDList": {"$in": group_id_list_including_subgroups}}, {"_id": 1})
        exp_id_list = [e["_id"] for e in exps]
    sample_id_list = []
    if not project_wide and ("Samples" in displayed_categories or "all" in displayed_categories):
        samples = get_db().db["Sample"].find({"GroupIDList": {"$in": group_id_list_including_subgroups}}, {"_id": 1})
        sample_id_list = [s["_id"] for s in samples]

    if project_wide:
        researchitem_id_list = []
    else:
        query = {"GroupIDList": {"$in": group_id_list_including_subgroups},
    ...
    exp_id_list, sample_id_list, researchitem_id_list = filter_items(project_id, group_id_list_including_subgroups,
                                                                     unfiltered_group_id_list_including_subgroups,
                                                                     name_query_groups, exp_id_list, sample_id_list,
                                                                     researchitem_id_list, experiments_filter,
                                                                     samples_filter, researchitems_filter, name_filter,
                                                                     combo_filter, numeric_filter, checks_filter,
                                                                     date_filter, date_created, text_filter,
                                                                     include_linked, cat_id_list,
                                                                     project_wide, displayed_categories)
    return name_query_groups, show_as_empty_groups, exp_id_list, sample_id_list, researchitem_id_list


def prepare_result_group_id_list_and_group_item_mapping(top_level_group_id_list, name_query_groups,
    ...
    group_item_mapping = {}
    if exp_id_list:
        exps = get_db().db["Experiment"].find({"_id": {"$in": exp_id_list}},
    ...
    if sample_id_list:
        samples = get_db().db["Sample"].find({"_id": {"$in": sample_id_list}},
                                             {"_id": 1, "GroupIDList": 1, "Name": 1, "ShortID": 1})
        for sample in samples:
    ...
    if researchitem_id_list:
        researchitems = get_db().db["ResearchItem"].find({"_id": {"$in": researchitem_id_list}},
                                                         {"_id": 1, "GroupIDList": 1, "Name": 1, "ShortID": 1,
...
def filter_groups(project_id, group_id_list, groups_filter, name_filter, combo_filter, numeric_filter, checks_filter,
                  date_filter,
                  date_created, text_filter, project_wide=False):
    show_as_empty_groups = []
    name_query_groups = []
    unfiltered_group_id_list_including_subgroups = []
    ...
    if name_filter:
        if project_wide:
            name_query = {"ProjectID": project_id,
                          "NameLower": {"$regex": name_filter.lower()}}
        else:
            name_query = {"_id": {"$in": group_id_list_including_subgroups},
                          "NameLower": {"$regex": name_filter.lower()}}
    ...
    return group_id_list_including_subgroups, name_query_groups, show_as_empty_groups, unfiltered_group_id_list_including_subgroups


def filter_items(project_id, group_id_list_including_subgroups, unfiltered_group_id_list_including_subgroups,
                 name_query_groups, exp_id_list,
                 sample_id_list,
                 researchitem_id_list,
                 exp_filter, sample_filter, researchitem_filter,
                 name_filter, combo_filter, numeric_filter, checks_filter, date_filter,
                 date_created, text_filter, include_linked, category_id_list, project_wide, displayed_categories):
    """
    ...
    filter_applied = check_if_filter_applied(name_filter=name_filter, combo_filter=combo_filter, numeric_filter= numeric_filter,
                            samples_filter=sample_filter, experiments_filter=exp_filter, researchitems_filter=researchitem_filter,
                            checks_filter=checks_filter, date_filter=date_filter, groups_filter=[], date_created=date_created,
                            text_filter=text_filter)
    ...
    if not filter_applied:
        if project_wide:
            ...
            if "Experiments" in displayed_categories or "all" in displayed_categories:
                exps = get_db().db["Experiment"].find({"GroupIDList": {"$in": group_id_list_including_subgroups}},
                                                      {"_id": 1})
                exp_id_list = [e["_id"] for e in exps]

            if "Samples" in displayed_categories or "all" in displayed_categories:
                samples = get_db().db["Sample"].find({"GroupIDList": {"$in": group_id_list_including_subgroups}},
                                                     {"_id": 1})
                sample_id_list = [s["_id"] for s in samples]
            if category_id_list:
                query = {"GroupIDList": {"$in": group_id_list_including_subgroups},
    ...
    if name_filter:
        if project_wide:
            name_query = {"$or": [
                {"ProjectID": project_id, "NameLower": {"$regex": name_filter.lower()}},
                {"GroupIDList": {"in": name_query_groups}}
            ]}
            index_to_be_used = "project_namelower"
        else:
            name_query = {"$or": [
                {"GroupIDList": {"$in": unfiltered_group_id_list_including_subgroups},
                 "NameLower": {"$regex": name_filter.lower()}},
                {"GroupIDList": {"in": name_query_groups}}
            ]}
            index_to_be_used = "group_namelower"
        if "Experiments" in displayed_categories or "all" in displayed_categories:
            exps = get_db().db["Experiment"].find(name_query, {"_id": 1}).hint(index_to_be_used)
            exp_id_list = [e["_id"] for e in exps]
    ...
    if exp_filter or sample_filter or researchitem_filter:
        if project_wide:
            if "all" in exp_filter:
                if not exp_id_list:
                    if "Experiments" in displayed_categories or "all" in displayed_categories:
                        exps = get_db().db["Experiment"].find({"ProjectID": project_id}, {"_id": 1})
                        exp_id_list = [e["_id"] for e in exps]
            else:
                if not exp_id_list:
            ...
            if "all" in sample_filter:
                if not sample_id_list:
                    if "Samples" in displayed_categories or "all" in displayed_categories:
                        samples = get_db().db["Sample"].find({"ProjectID": project_id}, {"_id": 1})
                        sample_id_list = [s["_id"] for s in samples]
            else:
                if not sample_id_list:
            ...
            if "all" in researchitem_filter:
                if not researchitem_id_list:
                    if category_id_list:
                        researchitems = get_db().db["ResearchItem"].find({"ProjectID": project_id,
                                                                         "ResearchCategoryID": {"$in": category_id_list}},
                                                                         {"_id": 1})
                        researchitem_id_list = [r["_id"] for r in researchitems]
            else:
                if not researchitem_id_list:
                    researchitem_id_list = [bson.ObjectId(r) for r in researchitem_filter]
    ...
    if date_created_query or field_data_query:
        if project_wide:
            ...
            if "Experiments" in displayed_categories or "all" in displayed_categories:
                ...
                print("Experiments", time.time() - s,len(exp_id_list))
    ...
    extra_exp_id_list, extra_sample_id_list, extra_researchitem_id_list = get_linked_items_recursive(
...
def get_valid_categories(project_id, displayed_categories):
    ...
    categories = get_db().db["ResearchCategory"].find({"ProjectID": project_id}, {"_id": 1, "Name": 1}).sort("Name")
    category_mapping = {c["_id"]: c["Name"] for c in categories}
    cat_id_list = []
    for cat_id in category_mapping:
        cat_name = category_mapping[cat_id]
...
def get_page_item_mapping_dashboard(dataview_id, project_id, name_filter, combo_filter,
                                    numeric_filter, groups_filter,
                                    samples_filter, experiments_filter, researchitems_filter,
                                    checks_filter, date_filter, date_created, text_filter,
                                    include_linked, displayed_categories):

    filter_applied = check_if_filter_applied(name_filter, combo_filter, numeric_filter, groups_filter,
                                             samples_filter, experiments_filter, researchitems_filter, checks_filter,
                                             date_filter, date_created, text_filter)
    if not filter_applied:
        return {
            "maxPage": 1,
    ...
    project_wide = True
    toplevel_groups = get_db().db["Group"].find({"ProjectID": project_id, "GroupID": None}, {"_id": 1})
    top_level_group_id_list = [g["_id"] for g in toplevel_groups]

    name_query_groups, show_as_empty_groups, exp_id_list, sample_id_list, researchitem_id_list = get_filtered(
        project_id, top_level_group_id_list, name_filter, combo_filter, numeric_filter,
        groups_filter, samples_filter, experiments_filter, researchitems_filter,
        checks_filter, False, date_filter, date_created, text_filter,
        displayed_categories, include_linked, project_wide, cat_id_list)

    exps = list(get_db().db["Experiment"].find({"_id": {"$in": exp_id_list}}, {"_id": 1, "Date": 1}))
    samples = list(get_db().db["Sample"].find({"_id": {"$in": sample_id_list}}, {"_id": 1, "Date": 1}))
    researchitems = list(
        get_db().db["ResearchItem"].find({"_id": {"$in": researchitem_id_list}}, {"_id": 1, "Date": 1}))
    ...
    Update.update("DataView", "ItemIDList", item_id_list, dataview_id, no_versioning=True)

    result_dict = {"maxPage": page - 1, "pageItemMapping": page_item_mapping}
    return result_dict


def check_if_filter_applied(name_filter, combo_filter,
                            numeric_filter, groups_filter,
                            samples_filter, experiments_filter, researchitems_filter,
                            checks_filter, date_filter, date_created, text_filter):

    if (name_filter or combo_filter or groups_filter or samples_filter or experiments_filter or
            researchitems_filter or numeric_filter or date_filter or date_created or
            text_filter or checks_filter):
        return True
    else:
        return False
