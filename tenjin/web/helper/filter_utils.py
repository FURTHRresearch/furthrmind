    ...
    if experiment_filter or sample_filter or research_item_filter:
        if experiment_filter:
            if "all" in experiment_filter:
                ...
                if exp_ids:
                    exp_ids = [
                        ObjectId(e) for e in experiment_filter if ObjectId(e) in exp_ids
                    ]
                else:
        ...
        if sample_filter:
            if "all" in sample_filter:
                ...
                if sample_ids:
                    sample_ids = [
                        ObjectId(s) for s in sample_filter if ObjectId(s) in sample_ids
                    ]
                else:
        ...
        if research_item_filter:
            if "all" in research_item_filter:
                if not research_item_ids:
                    research_item_ids = "all"
            else:
                if research_item_ids:
                    research_item_ids = [
                        ObjectId(r)
                        for r in research_item_filter
                        if ObjectId(r) in research_item_ids
                    ]
                else:
...
def get_field_data_query_objects(
    ...
    if combo_filter:
        ...
        for cf in combo_filter:
            values_str = [i["id"] for i in cf["values"]]
            values = []
            for v in values_str:
                try:
...
async def get_all_parent_groups(group_ids: set):
    ...
    subgroup_ids = set([s["_id"] for s in subgroups])
...
async def field_data_query_coroutine(
    ...
    if item_ids:
        query["_id"] = {"$in": list(item_ids)}
    fielddata_cursor = db[collection].find(query, {"_id": 1, "GroupIDList": 1})
...
async def get_linked_items_recursive(include_linked, item_id_list, retrieve_linked_mapping=False):
    ...
    if isinstance(include_linked, str):
        if include_linked.lower() == "none":
            return extra_item_id_list, total_linked_mapping

    if include_linked in ["0", 0]:
        return extra_item_id_list, total_linked_mapping
    
    if include_linked == "direct":
        extra_item_id_list, linked_mapping = await get_linked_items(item_id_list)
        if retrieve_linked_mapping:
            for item_id in item_id_list:
                total_linked_mapping[item_id] = [[]]
            for item_id in linked_mapping:
                total_linked_mapping[item_id][0] = linked_mapping[item_id]
        return extra_item_id_list, total_linked_mapping
    ...
    if include_linked == "all" or number_of_layer:
        ...
        while True:
            for item_id in item_id_list:
                total_linked_mapping[item_id].append([])
            temp_item_id_list, linked_mapping = await get_linked_items(
                found_item_id_list
            )
            if retrieve_linked_mapping:
                if counter == 0:
                    for parent_id in linked_mapping:
                        item_ids = linked_mapping[parent_id]
                        for item_id in item_ids:
                            if item_id not in item_parent_mapping:
                                item_parent_mapping[item_id] = []
                            item_parent_mapping[item_id].append(parent_id)
                            total_linked_mapping[parent_id][-1].append(item_id)
                else:
                    for sub_parent_id in linked_mapping:
                        parent_ids = item_parent_mapping[sub_parent_id]
                        item_ids = linked_mapping[sub_parent_id]
                        for item_id in item_ids:
                            if item_id not in item_parent_mapping:
                                item_parent_mapping[item_id] = []
                            for parent_id in parent_ids:
                                if parent_id not in item_parent_mapping[item_id]:
                                    item_parent_mapping[item_id].append(parent_id)
                                if item_id not in total_linked_mapping[parent_id][-1]:
                                    total_linked_mapping[parent_id][-1].append(item_id)

            found_item_id_list = list(set(temp_item_id_list) - extra_item_id_set)
            extra_item_id_set.update(temp_item_id_list)

            if include_linked == "all" and not found_item_id_list:
                break
            counter += 1
        ...
        return (list(extra_item_id_set), total_linked_mapping)
...
async def compare_and_filter_items(
    ...
    for r in results:
        ...
        coroutines.append(get_linked_items_recursive(include_linked, item_ids, retrieve_linked_mapping=False))
